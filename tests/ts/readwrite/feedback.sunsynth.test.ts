...
describe("Reading the feedback.sunsynth file", () => {
  const filePath = "tests/files/feedback.sunsynth"
  let synth: Synth
  beforeAll(() => {
    // read, write, read
  ...
  test("has correct properties, controllers, and options", () => {
    const mod = synth.module as m.Feedback.Module
    expect(mod.flags).toEqual(6291537)
    expect(mod.name).toEqual("Feedback")
    const { c } = mod
    expect(c.volume).toEqual(1708)
    expect(c.channels).toEqual(m.Feedback.Channels.Mono)
  })
})
