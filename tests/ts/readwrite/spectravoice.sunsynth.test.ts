...
describe("Reading the spectravoice.sunsynth file", () => {
  ...
  test("has correct properties, controllers, and options", () => {
    ...
    expect(mod.behavior?.harmonicFrequencies).toEqual(
      Uint16Array.from([
        17916, 7063, 5426, 7235, 18002, 10594, 775, 20586, 10853, 16279, 14729, 4479,
        12231, 3876, 19036, 21275,
      ])
    )
    expect(mod.behavior?.harmonicVolumes).toEqual(
      Uint8Array.from([
        114, 203, 245, 42, 191, 102, 243, 195, 184, 59, 224, 144, 213, 182, 21, 238,
      ])
    )
    expect(mod.behavior?.harmonicWidths).toEqual(
      Uint8Array.from([
        99, 242, 70, 26, 245, 9, 43, 10, 219, 135, 94, 235, 66, 124, 114, 16,
      ])
    )
  ...
  test("has correct chunks when written out", () => {
    ...
    expectChunk({
      ...
      value: new Uint8Array([
        252, 69, 151, 27, 50, 21, 67, 28, 82, 70, 98, 41, 7, 3, 106, 80, 101, 42, 151,
        63, 137, 57, 127, 17, 199, 47, 36, 15, 92, 74, 27, 83,
      ]),
    })
