...
describe("Reading the eq.sunsynth file", () => {
  const filePath = "tests/files/eq.sunsynth"
  let synth: Synth
  beforeAll(() => {
    // read, write, read
  ...
  test("has correct properties, controllers, and options", () => {
    const mod = synth.module as m.Eq.Module
    expect(mod.flags).toEqual(81)
    expect(mod.name).toEqual("EQ")
    const { c } = mod
    expect(c.low).toEqual(402)
    expect(c.middle).toEqual(476)
    expect(c.high).toEqual(445)
    expect(c.channels).toEqual(m.Eq.Channels.Mono)
  })
})
